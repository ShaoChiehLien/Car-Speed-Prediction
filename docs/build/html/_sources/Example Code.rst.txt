...
Read in all the frames two by two, resize them two 0.5 of the original size, calculate the optical flow between two frames, and slice the output into 8 by 6 matrix, and flatten it into 1*48 array. If there are n frames, the output feature.txt would be a size n*48 matrix.
